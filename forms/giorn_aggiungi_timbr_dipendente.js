			...
			try 
			{
				...
				if(vCausale != null)
				{
					...
					if(recCaus)
					{
						...
						recCaus.nr_badge = globals.getNrBadge(idLavoratore,vTimbratura);
				...
				else
				{
					...
					if(rec)
					{
						...
						rec.senso = vSenso;
						rec.nr_badge = globals.getNrBadge(idLavoratore,vTimbratura);
						rec.sensocambiato = 0;
						rec.timbeliminata = 0;
						rec.ggsucc = 0;
...
function onActionInfo(event) {
	...
	var frm = forms.giorn_timbr_commesse_tbl;
	var fs = frm.foundset;
	...
	globals.ma_utl_showFormInDialog(frm.controller.getName(),'Visualizza timbrature del giorno');
}
